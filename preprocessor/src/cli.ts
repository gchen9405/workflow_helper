...
import type { Question } from "./pipeline/questions.js";
...
class ReadlineClarificationIO implements ClarificationIO {
  constructor(private readonly rl: readline.Interface) {}

  async askBatch(questions: Question[], round: number): Promise<BatchAnswers> {
    console.log(
...
const silentIO: ClarificationIO = {
 ...
 */
function openClarificationIO(): { io: ClarificationIO; close: () => void } | null {
  if (process.stdin.isTTY) {
