 ...
 * semanticCheck, proving they are shaped like legal model output. Handlers
 * are keyed by the stage's task label — `triage`, `normalization`,
 * `extraction`, `answer application`, `profiling`, `summary` — so one fake
 * can stand in for the endpoint across all three components.
 */
import {
...
export class FakeLlm implements LlmClient {
  calls = 0;
  /** Task labels in call order, for sequencing assertions. */
  labels: string[] = [];
  ...
  async structured<T>(options: StructuredCallOptions<T>): Promise<T> {
    ...
    this.calls++;
    this.labels.push(options.taskLabel);
...
export function requestedIds(options: StructuredCallOptions<unknown>): string[] {
  const text = options.user.map((part) => (part.type === "text" ? part.text : "")).join("\n");
  return [...text.matchAll(/^- id: (.+)$/gm)].map((m) => m[1]);
}

/** A profiling handler that answers each requested id from a canned map. */
