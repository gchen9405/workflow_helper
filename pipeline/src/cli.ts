...
async function bootstrap(): Promise<number> {
  try {
    ...
    if (/workflow-(preprocessor|recommender|narrator)/.test(message) && /find|resolve/i.test(message)) {
      console.error(
        [
          "The sibling workflow-preprocessor / workflow-recommender / workflow-narrator packages are not installed/built yet.",
          "From the pipeline folder, run:",
          "",
          "  npm run setup",
          "",
          "(builds ../preprocessor, ../recommender and ../narrator, then installs this package's dependencies)",
        ].join("\n"),
      );
      return 1;
    }
